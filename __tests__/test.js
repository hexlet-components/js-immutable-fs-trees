...
test('reduce', () => {
  ...
  const actual = reduce((n, acc) => acc + 1, tree, 0);
  expect(actual).toEqual(6);

  const actual2 = reduce((n, acc) => (n.type === 'file' ? acc + 1 : acc), tree, 0);
  expect(actual2).toEqual(2);

  const actual3 = reduce((n, acc) => (n.type === 'directory' ? acc + 1 : acc), tree, 0);
